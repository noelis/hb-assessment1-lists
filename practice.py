...
def long_words(words):
    ...
    # Answer 1: (passing test with for loops)
    # four_letter_words = []
    ...
    # Answer 2: (passing test with list comprehension)
...
def n_long_words(words, n):
    ...
    #Answer 2: (w/ list comprehension)
    n_length_words = [word for word in words if len(word) > n]
    return n_length_words
...
def halvesies(numbers):
    ...
    half_numbers = [num/2.0 for num in numbers]
    return half_numbers
...
def mult_numbers(numbers):
    ...
    for number in numbers:
        multiplied_total = number * multiplied_total
...
def average(numbers):
    ...
    if numbers == []:
        ...
        for number in numbers:
            mean_total += number
    return float(mean_total) / len(numbers)
...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    reverse_items = [item for item in items[::-1]]
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    For example::
        ...
        ['I', 'love', 'cookies']
     """
    # I found 2 ways to do this but none of them were passing all of the doctests originally. After a few hours of
    # debugging, I realized that removing the return did the trick!

    # Answer A: replaces the list slice in the original order with the reverse order list slice.
    items[0:] = items[::-1]

    # Answer B: does something similar as above, but uses an algorithm in the form of a for loop instead.

    # length_of_items = len(items)
    # for i in range(length_of_items/2):
    #     items[i], items[length_of_items - 1 - i] = items[length_of_items - 1 - i], items[i]
    #     items
...
def duplicates(items):
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'apple', 'berry']
    """
    # My first instinct was to try to only use lists to solve this, but it wasn't passing all of
    # the doctests (I kept getting 1 remaining failed test, and I couldn't figure out why.)
    # I kept some of the original logic and used the set's removal of duplicates to pass all the tests.
    ...
    convert_to_set = set(duplicate_items)
    duplicates = list(convert_to_set)
    duplicates.sort()
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    letter_index = []
    for word in words:
        count = -1    # Used -1 so that after the first loop it can be increased to the count of 0 (index of 0)
        for character in word:
            ...
            if character == letter:
                ...
                break  # Used to stop loop from adding None for each time it looped over each character in the word.
    return letter_index
